/**
 * Chat window — message list, text input, file attach, file browser panel.
 * Messages right-aligned for "You", left-aligned for "Peer".
 */

import { useState, useRef, useEffect, useCallback } from 'react';
import type { ChatMessage, SessionFile } from '../hooks/useRoom';
...
interface ChatWindowProps {
  ...
  onDeleteSessionFile: (id: string) => void;
  error: string | null;
}
...
export function ChatWindow({
  ...
  // Auto-open file panel when a new file arrives
  useEffect(() => {
    if (sessionFiles.length > 0) setShowFiles(true);
  }, [sessionFiles.length]);
  ...
  return (
    <div className="flex h-screen bg-gray-950 text-white overflow-hidden">
      ...
      <div className="flex flex-col flex-1 min-w-0">
        {/* Header */}
        <div className="flex items-center gap-3 px-4 py-3 bg-gray-900 border-b border-white/5 shrink-0">
          ...
          <button
            ...
            className={`ml-2 p-1.5 rounded-lg transition-colors relative ${
              showFiles
          ...
          >
            ...
            {sessionFiles.length > 0 && !showFiles && (
        ...
        {/* Error banner */}
        {(error || fileError) && (
          <div className="px-4 py-2 bg-red-900/50 border-t border-red-800 text-red-300 text-sm shrink-0">
            {error || fileError}
          </div>
        )}

        {/* Input area */}
        <div className="px-4 py-3 bg-gray-900 border-t border-white/5 shrink-0">
          <div className="flex items-end gap-2">
            ...
            <textarea
              ...
              rows={1}
              className="flex-1 bg-gray-800 text-white placeholder-gray-500 rounded-xl px-3 py-2 text-sm resize-none border border-white/5 focus:outline-none focus:border-blue-500/50 transition-colors min-h-9.5 max-h-32"
